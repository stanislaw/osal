...
int32 OS_QueueCreate_Impl (uint32 queue_id, uint32 flags)
{
    ...
    */
   /// TODO-MAC: temporary hack until the posix-addons/mqueue is fixed.
   snprintf(name, sizeof(name), "/tmp/%d.%s", (int)getpid(), OS_global_queue_table[queue_id].name_entry);
...
int32 OS_CountSemCreate_Impl (uint32 sem_id, uint32 sem_initial_value, uint32 options)
{
    ...
    if (mac_sem2_init(&OS_impl_count_sem_table[sem_id].id, 0, sem_initial_value) < 0)
    {
        return OS_SEM_FAILURE;
    }
...
{
    if (mac_sem2_destroy(&OS_impl_count_sem_table[sem_id].id) < 0)
    {
        return OS_SEM_FAILURE;
    }
...
{
    if (mac_sem2_post(&OS_impl_count_sem_table[sem_id].id) < 0)
    {
        return OS_SEM_FAILURE;
    }
...
{
    if (mac_sem2_wait(&OS_impl_count_sem_table[sem_id].id) < 0)
    {
        return OS_SEM_FAILURE;
    }
...
int32 OS_CountSemTimedWait_Impl ( uint32 sem_id, uint32 msecs )
{
   ...
   if (mac_sem2_timedwait(&OS_impl_count_sem_table[sem_id].id, &ts) == 0)
   {
       result = OS_SUCCESS;
   }
   else if (errno == ETIMEDOUT)
...
int32 OS_CountSemGetInfo_Impl (uint32 sem_id, OS_count_sem_prop_t *count_prop)
{
    ...
    if (mac_sem2_getvalue(&OS_impl_count_sem_table[sem_id].id, &sval) < 0)
    {
        return OS_SEM_FAILURE;
    }
...
int32 OS_ConsoleCreate_Impl(uint32 local_id)
{
    ...
    if (local_id == 0)
    {
        ...
        if (local->is_async)
        {
            if (mac_sem2_init(&OS_impl_console_table[local_id].data_sem, 0, 0) < 0)
            {
                return_code = OS_SEM_FAILURE;
            }
            else
            {
                ...
                if (return_code != OS_SUCCESS)
                {
                    mac_sem2_destroy(&OS_impl_console_table[local_id].data_sem);
                }
            }
        }
    }
